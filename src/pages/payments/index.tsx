...
import type { GymPayment, Athlete, Gym } from '@/types'
...
export function PaymentsPage() {
  ...
  const columns = [
    {
      ...
      cell: ({ row }: { row: { original: GymPayment } }) => (
        <span>{formatDate(row.original.day_payed)}</span>
      ),
    },
    {
      header: 'Monto',
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: GymPayment } }) => {
        const athlete = athletes.find(a => a.id === row.original.athlete_id)
        return athlete ? `${athlete.name} ${athlete.surname}` : row.original.athlete_id
      },
    },
    {
      header: 'Referencia',
      ...
      cell: ({ row }: { row: { original: GymPayment } }) => (
        <span>{row.original.payment_reference || '-'}</span>
      ),
    },
    {
      header: 'Acciones',
  ...
  const filteredPayments = payments.filter(
    (payment) => {
      ...
      return reference.toLowerCase().includes(search.toLowerCase())
    }
  )
